...
public class World implements IWorld {
    ...
    public World(int width, int height, TheGameEngine engine, GameImage gameImage, GameAudio gameAudio, GameOptions options){
        Engine = engine;
        GameImage = gameImage;
        GameAudio = gameAudio;
        GameOptions = options;
    ...
    private ArrayList<CollisionCheckInfo> MoveObjects(KeyEvent event){
        ...
        // Advance each snake in the world and gather the collision info.
        for (IWorldObject object : Objects) {
            var newCell = object.Move(event.getKeyCode());
            if (newCell != null) {
                AudioRequests.add(new AudioRequest(WorldObjectType.Snake));
        ...
        for (CollisionCheckInfo collisionCheckInfo : toCheck) {
            ...
            if (toRemove == null) {
                ...
                if (info != null) {
                    ...
                    if (info.Remove != null && info.Remove.WhoAmI() == WorldObjectType.Snake &&
    ...
    public void PlayAudio(){
        if (AudioRequests.size() > 0) {
            ...
            else if (request.Type == WorldObjectType.Snake) {
                // Snake
